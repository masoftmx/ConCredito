...
function ClientesDetalleCtrl($scope, $meteor, $reactive, $state, toastr, $stateParams, Estatus) {
	...
	this.helpers({
		...
		compras : () => {
			var compras = Ventas.find({cliente_id : this.getReactively("cliente._id")},{ $sort: { fechaCreacion : -1}}).fetch();
			if(compras)
	...
	this.actualizar = function(cliente,form){
		...
		cliente.profile.nombreCliente = nombre + apPaterno + apMaterno;
  ...
  this.ver = function(compra){
	  console.log(compra);
	  Meteor.apply("bitacoraVentaFolio", [compra.folios[0], compra.sucursal_id], function(error, result){
		  if(result){
			  console.log(result);
			  rc.compraSeleccionada = result;
			  $scope.$apply();
			  $('#bitacoraCompra').modal('show');
			  
		  }
	  });
	  
/*
	  Meteor.apply("bitacoraVentaFolio", [$stateParams.folioVenta, $stateParams.sucursal_id], function(error, result){
		  if(result){
			  rc.compraSeleccionada = result;
			  $scope.$apply();
		  }
...
*/
  ...
  this.getVentaPorTicket = function(folioVenta, sucursal_id){
	  var url = $state.href(
										"anon.ventaImprimir",
										{folioVenta : folioVenta, sucursal_id : sucursal_id},
										{newTab : true}
									);
